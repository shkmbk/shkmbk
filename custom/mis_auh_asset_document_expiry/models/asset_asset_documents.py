...
class AccountAssetDocument(models.Model):
            ...
            if i.expiry_date:
                if i.notification_type == 'single':
                    ...
                    if date_now == exp_date:
                        mail_content = "  Hello  " + i.expiry_reminder_user_id.name + ",<br>Your Document " + i.name + "is going to expire on " + \
                    ...
                    if date_now == exp_date or date_now == i.expiry_date:
                        mail_content = "  Hello  " + i.expiry_reminder_user_id.name + ",<br>Your Document " + i.name + \
    ...
    asset_ref = fields.Many2one('account.asset', srting="Asset", copy=False)
...
class AssetAsset(models.Model):
    ...
    def _document_count(self):
        for each in self:
            document_ids = self.env['account.asset.document'].sudo().search([('asset_ref', '=', each.id)])
    ...
    def document_view(self):
        ...
        return {
            'name': _('Documents'),
            'domain': domain,
            'res_model': 'account.asset.document',
            ...
            'limit': 80,
            'context': "{'default_asset_ref': %s}" % self.id
        }

    document_count = fields.Integer(compute='_document_count', string='# Documents')


class AssetAssetAttachment(models.Model):
    _inherit = 'ir.attachment'

    doc_attach_rel = fields.Many2many('account.asset.document', 'doc_attachment_id', 'attach_id3', 'doc_id',
                                      string="Attachment", invisible=1)
    attach_rel = fields.Many2many('asset.document', 'attach_id', 'attachment_id3', 'document_id',
                                  string="Attachment", invisible=1)
