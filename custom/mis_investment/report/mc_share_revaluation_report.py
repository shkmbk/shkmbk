...
class MCShareRevaluationReport(models.AbstractModel):
    _name = "report.mis_investment.report_mc_share_revaluation_document"
    _description = "Multi-Currency Revaluation Report"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for shr in objshare:
            ...
            closingprice = self.get_last_closing_amount(reval_id, shr.id, to_date)*c_rate
            realize_profit = self.get_realized_profit_loss(shr.id, from_date, to_date)*c_rate
            dividend = self.get_dividend(shr.id, from_date, to_date)*c_rate
            brokerage_expense = self.get_brokerage_expense(shr.id,from_date, to_date)*c_rate
            unrelize=self.get_last_unrelize_amount(reval_id, shr.id, to_date)*c_rate
            closing_amount=self.get_closing_amount(shr.id, dtfilter)*c_rate
            cost=0.00
            if qty>0.0:
