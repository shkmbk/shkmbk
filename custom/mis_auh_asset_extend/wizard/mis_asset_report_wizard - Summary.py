...
class MisAssetCustomReport(models.TransientModel):
    ...
    def _getdomainfilter_move(self, assetid, isstart, isenddate, startdate, enddate):
        return [('asset_id', '=', assetid),  self._get_datefilter(isstart, isenddate, startdate, enddate),
...
#                for mvrec in objmove_line:
    ...
    def print_asset_xlsx(self):
        ...
        column_width = 14
        worksheet.set_column(col, col, column_width)
        worksheet.write(count, col, 'Opening Value', wbf['content_border_bg'])
        col += 1
        column_width = 14
        worksheet.set_column(col, col, column_width)
        ...
        column_width = 16
        worksheet.set_column(col, col, column_width)
        worksheet.write(count, col, 'NBV '+str(self.from_date), wbf['content_border_bg'])
        ...
        for rec in objasset:
            ...
            # Purchase Date
            col += 1
            worksheet.write(count, col, str(rec.asset_purchase_date), wbf['content_border'])
            # QTY
            col+=1
            ...
            # Opening Balance
            col+=1
            sum_opening += rec.book_value
            worksheet.write(count, col, rec.book_value,  wbf['content_float_border'])
            # Addition
            col += 1
            ...
            # DEPRECIABLE VALUE
            col += 1
            sum_depreciable+=rec.value_residual
            worksheet.write(count, col, rec.value_residual, wbf['content_float_border'])
            ...
            if rec.method_period==1:
                ...
                strmy='Months'
            col += 1
            worksheet.write(count, col, str(rec.method_number) + ' ' + strmy, wbf['content_border'])

            # Depreciation Opening
            col += 1
            sum_depcreciable_opening+=rec.book_value
            worksheet.write(count, col, rec.book_value, wbf['content_float_border'])
