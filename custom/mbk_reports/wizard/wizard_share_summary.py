...
class MbkStockSummary(models.TransientModel):
    ...
    def print_share_summary_pdf(self):
        ...
        if not objstockval:
            raise UserError(_('There are no transaction found for selected parameters'))
        ...
        wbf['content_qty_border_bg'] = workbook.add_format(
        ...
        for prd in objstockval.product_id:
            sm = self.env['stock.valuation.layer'].search(
                [('company_id', '=', self.env.company.id), self._get_product(), ('product_id', '=', prd.id)])
            opening_qty = 0.0
            opening_amt = 0.0
            in_qty = 0.0
            in_amt = 0.0
            ...
            for rec in sm:
                ...
                if self.from_date <= rec.create_date.date() <= self.to_date:
                    if rec.quantity > 0:
                        in_qty += rec.quantity
                        in_amt += rec.value
                # Out Qty
    ...
    def view_share_summary(self):
        data = {}
        data['from_date'] = self.from_date
        data['to_date'] = self.to_date
        data['product_id'] = self.product_id.id
        data['header_date'] = self.to_date.strftime("%d-%m-%Y")
        report = self.env.ref('mbk_reports.stock_summary')
