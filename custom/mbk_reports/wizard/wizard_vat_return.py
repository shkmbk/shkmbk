...
class MbkVATReturn(models.TransientModel):
    ...
    def print_vat_return_xls(self):
        ...
        worksheet3.set_column(col, col, 27)
        ...
        col += 1
        worksheet1.set_column(col, col, 35)
        ...
        worksheet3.set_column(col, col, 64)
        ...
        worksheet4.set_column(col, col, 17)
        ...
        worksheet3.set_column(col, col, 17)
        worksheet3.write(count, col, 'VAT Rate', wbf['content_border_bg'])
        worksheet4.set_column(col, col, 25)
        ...
        worksheet3.set_column(col, col, 21)
        ...
        worksheet1.write_formula('M%s' % (in_inv_count+2), '=SUM(M2:M%s)' % (in_inv_count+1), wbf['content_border_bg_total'])
        worksheet1.write_formula('O%s' % (in_inv_count+2), '=SUM(O2:O%s)' % (in_inv_count+1), wbf['content_border_bg_total'])
        worksheet1.write(in_inv_count+1, 13, '', wbf['content_border_bg_total'])
        worksheet1.write(in_inv_count+1, 15, '', wbf['content_border_bg_total'])
        worksheet1.write(in_inv_count+1, 16, '', wbf['content_border_bg_total'])

        worksheet3.merge_range('A%s:G%s' % (out_inv_count+2, out_inv_count+2), 'Total', wbf['content_border_bg_total'])
        worksheet3.write_formula('H%s' % (out_inv_count+2), '=SUM(H2:H%s)' % (out_inv_count+1), wbf['content_border_bg_total'])
        worksheet3.write_formula('J%s' % (out_inv_count+2), '=SUM(J2:J%s)' % (out_inv_count+1), wbf['content_border_bg_total'])
