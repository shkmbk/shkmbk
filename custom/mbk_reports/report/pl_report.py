...
class PL_Report(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        self._cr.execute("""SELECT AG.Name AS particulars,
                            ...
                            WHERE  AML.parent_state='posted' AND (AML.date BETWEEN '"""+str(year_from)+"""' AND '"""+str(year_to)+"""') AND AML.company_id=3 AND A.internal_group in ('income') AND (AML.analytic_account_id = '"""+str(analytic_id)+"""' OR '"""+str(analytic_id)+"""'=0)
        ...
        self._cr.execute("""SELECT AG.Name AS particulars,
                            ...
                            INNER JOIN account_account A ON AML.account_id=A.id
                            INNER JOIN account_group AG ON A.group_id= AG.id
                            WHERE  AML.parent_state='posted' AND (AML.date BETWEEN '"""+str(year_from)+"""' AND '"""+str(year_to)+"""') AND AML.company_id=3 AND A.internal_group in ('expense') and A.user_type_id<>16 AND (AML.analytic_account_id = '"""+str(analytic_id)+"""' OR '"""+str(analytic_id)+"""'=0)
                            GROUP BY AG.Name ORDER BY 2 DESC""")
        ...
        self._cr.execute("""SELECT 'Depreciation and Amortization' AS particulars,
                            ...
                            INNER JOIN account_account A ON AML.account_id=A.id
                            INNER JOIN account_group AG ON A.group_id= AG.id
                            WHERE  AML.parent_state='posted' AND (AML.date BETWEEN '"""+str(year_from)+"""' AND '"""+str(year_to)+"""') AND AML.company_id=3 AND A.internal_group in ('expense') and A.user_type_id=16 AND (AML.analytic_account_id = '"""+str(analytic_id)+"""' OR '"""+str(analytic_id)+"""'=0) ORDER BY 2 DESC""")
