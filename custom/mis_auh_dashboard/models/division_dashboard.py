...
class DivisionDashBoard(models.Model):
    ...
    def get_division_profit(self, *division_id):
        ...
        analytic_id = int(division_id[1])

        header = month_last_day.strftime("%B %Y").upper()

        self._cr.execute("""SELECT  SUM(AML.credit-AML.debit) AS pl_this_year,
                            SUM(CASE WHEN AML.date BETWEEN '"""+str(month_first_day)+"""' AND '"""+str(month_last_day)+"""' THEN AML.credit-AML.debit ELSE 0.00 END) AS pl_this_month,                           
                            SUM(CASE WHEN A.internal_group='income' THEN AML.credit-AML.debit ELSE 0.00 END) AS income_this_year,
                            SUM(CASE WHEN A.internal_group='income' AND AML.date BETWEEN '"""+str(month_first_day)+"""' AND '"""+str(month_last_day)+"""' THEN AML.credit-AML.debit ELSE 0.00 END) AS income_this_month,
                            SUM(CASE WHEN A.internal_group='expense' THEN AML.debit-AML.credit ELSE 0.00 END) AS expense_this_year,
                            SUM(CASE WHEN A.internal_group='expense' AND AML.date BETWEEN '"""+str(month_first_day)+"""' AND '"""+str(month_last_day)+"""' THEN AML.debit-AML.credit ELSE 0.00 END) AS expense_this_month,
                            SUM(CASE WHEN A.user_type_id<>16 THEN AML.credit-AML.debit ELSE 0.00 END) AS opl_this_year,
                            SUM(CASE WHEN A.user_type_id<>16 AND AML.date BETWEEN '"""+str(month_first_day)+"""' AND '"""+str(month_last_day)+"""' THEN AML.credit-AML.debit ELSE 0.00 END) AS opl_this_month,
                            ...
                            WHERE  AML.parent_state='posted' AND (AML.date BETWEEN '"""+str(year_first_day)+"""' AND '"""+str(month_last_day)+"""') AND AML.company_id=3 AND A.internal_group in ('income','expense') AND (AML.analytic_account_id = '"""+str(analytic_id)+"""' OR '"""+str(analytic_id)+"""'=0)""")
    ...
    def get_revenue(self, *division_id):
        ...
        self._cr.execute("""SELECT  AG.Name AS particulars, SUM(AML.credit-AML.debit) AS amount
                            ...
                            INNER JOIN account_account A ON AML.account_id=A.id
                            INNER JOIN account_group AG ON A.group_id= AG.id
                            WHERE  AML.parent_state='posted' AND (AML.date BETWEEN '""" + str(month_first_day) + """' AND '""" + str(
            month_last_day) + """') AND AML.company_id=3 AND A.internal_group in ('income') AND (AML.analytic_account_id = '""" + str(
    ...
    def get_expense(self, *division_id):
        ...
        self._cr.execute("""SELECT  AG.Name AS particulars, SUM(AML.debit-AML.credit) AS amount
                            ...
                            INNER JOIN account_group AG ON A.group_id= AG.id
                            WHERE  AML.parent_state='posted' AND (AML.date BETWEEN '""" + str(month_first_day) + """' AND '""" + str(
            month_last_day) + """') AND AML.company_id=3 AND A.internal_group in ('expense') AND (AML.analytic_account_id = '""" + str(
            analytic_id) + """' OR '""" + str(analytic_id) + """'=0) GROUP BY 1 ORDER BY 2 DESC""")
    ...
    def get_division_income_expense(self, *division_id):
        ...
        if analytic_id == 0:
            self._cr.execute("""SELECT  AAA.code AS month, SUM(AML.credit-AML.debit) AS profit,                                                      
                                ...
                                WHERE AML.parent_state='posted' AND (AML.date BETWEEN '""" + str(month_first_day) + """' AND '""" + str(
                month_last_day) + """') AND AML.company_id=3 AND A.internal_group in ('income','expense') GROUP BY 1 ORDER BY 2 DESC""")
            pl_table = self._cr.dictfetchall()
        else:
            self._cr.execute("""SELECT TO_CHAR(AML.date, 'MON') AS month, SUM(AML.credit-AML.debit) AS profit,                                                      
                                ...
                                INNER JOIN account_group AG ON A.group_id= AG.id
                                INNER JOIN account_analytic_account AAA ON AML.analytic_account_id=AAA.id
                                WHERE AML.parent_state='posted' AND (AML.date BETWEEN '""" + str(year_first_day) + """' AND '""" + str(
                month_last_day) + """') AND AML.company_id=3 AND A.internal_group in ('income','expense') AND (AML.analytic_account_id = '""" + str(analytic_id) + """')
        ...
        self._cr.execute("""SELECT TO_CHAR(AML.date, 'MON') AS y_labels, SUM(AML.credit-AML.debit) AS profit,                                                      
                            ...
                            INNER JOIN account_group AG ON A.group_id= AG.id
                            INNER JOIN account_analytic_account AAA ON AML.analytic_account_id=AAA.id
                            WHERE AML.parent_state='posted' AND (AML.date BETWEEN '""" + str(
            year_first_day) + """' AND '""" + str(
            month_last_day) + """') AND AML.company_id=3 AND A.internal_group in ('income','expense') AND (AML.analytic_account_id = '""" + str(
        ...
        for line in opl_table:
            y_opl.append(line['opl_table'])
            y_labels.append(line['y_labels'])
