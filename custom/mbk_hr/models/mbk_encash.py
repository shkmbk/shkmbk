...
class MbkEncash(models.Model):
    ...
    date = fields.Date('Date',default=fields.Date.to_string(date.today()), states={'draft': [('readonly', False)], 'verify': [('readonly', False)]}, readonly=True,
        help="Keep empty to use the period of the validation(Payslip) date.")    
    company_id = fields.Many2one('res.company', 'Company', required=True, index=True)
    date_to = fields.Date(string='As on Date', readonly=True, required=True, states={'draft': [('readonly', False)], 'verify': [('readonly', False)]})
    date_effective = fields.Date(string='Effective Date', readonly=True,store=True, required=True, states={'draft': [('readonly', False)], 'verify': [('readonly', False)]})
    ...
    encash_amount = fields.Float(string='Amount', readonly=True,compute='compute_encash_amount', store=True, default=False)
    ...
    balance_days = fields.Float(string='Balance Days', readonly=True, compute='compute_al_balance', store=True, default=False)
    ...
    def compute_sheet(self):
        ...
        objlopleave = self.env['hr.leave'].search([('employee_id','=',employee_id.id),('state','=','validate'),('holiday_status_id.unpaid','=',1),('request_date_from','<=',to_date)])
        c_lop=0.00
        ...
        #Annual Leave Taken
        objalleave = self.env['hr.leave'].search([('employee_id','=',employee_id.id),('state','=','validate'),('holiday_status_id','=',1),('request_date_from','<=',to_date)])
        c_alt=0.00
    ...
    def action_encash_done(self):
        ...
        if self.encash_no=='New':
            self.encash_no=self.env['ir.sequence'].with_context(force_company=self.company_id.id).next_by_code('mbk.encash') or _('New')
    ...
    def _onchange_employee(self):
        ...
        if date_to > al_provision_date:
            self.warning_message = _("This Encash computation can be erroneous! Provision entries not be generated after %s." %
                (al_provision_date))
        else:
